---
name: agent-main
description: "Comprehensive skill file covering frontend design/architecture, PRD generation, and autonomous task execution. Use when the agent needs to: (1) Build frontend interfaces with distinctive design, (2) Generate Product Requirement Documents (PRDs), (3) Execute autonomous task workflows, (4) Follow structured development protocols. Triggers include: 'create a prd', 'write prd for', 'plan this feature', 'requirements for', 'spec out', 'ULTRATHINK', frontend/UI development requests, and autonomous coding tasks."
---
...
```
/mnt/skills/user/agent-main/SKILL.md
```
...
- **bash_tool**: Execute shell commands, run scripts, install packages
- **view**: Read files, examine directories, view images
- **create_file**: Create new files with content
- **str_replace**: Edit existing files by replacing unique strings
- **web_search**: Search the web for current information
- **web_fetch**: Fetch full content from URLs
- **present_files**: Share files with users via download links
- **MCP Integrations**: Canva, Vercel, Indeed, Hugging Face, Google Drive, etc.
...
- Use conversation_search and recent_chats for historical context
...
4. Use present_files to share with user
...
- Use `bash_tool` for command execution, package installation, running scripts
- Use `view` to read files, examine directories, view images
- Use `create_file` for creating new files with content
- Use `str_replace` for editing existing files
- Use `web_search` and `web_fetch` for external information
- Use `present_files` to share completed work with users

### MCP Integration Commands

When MCP servers are connected:
- Use **Canva** tools for design creation and management
- Use **Vercel** tools for deployment and hosting
- Use **Google Drive** tools for document access
- Use **Hugging Face** tools for ML model discovery
- Use **Indeed** tools for job and company data

### Best Practices for Tools

1. **Check availability first**: Not all tools may be enabled for every conversation
2. **Handle errors gracefully**: Provide fallback behavior when tools fail
3. **Chain operations logically**: Read before write, explore before modify
4. **Verify after changes**: Confirm operations completed successfully
5. **Present final outputs**: Always use present_files for deliverables
...
### Working Directory
- Use `/home/claude` for all working files and temporary work
- Copy user uploads from `/mnt/user-data/uploads` when needed
- Place final outputs in `/mnt/user-data/outputs` for user access

### Output Delivery
Always use `present_files` tool to share completed files with users:
```
1. Create file in /home/claude or directly in /mnt/user-data/outputs
2. Call present_files with the file path
3. Provide concise summary of the deliverable
```
